...
public class OrderDao {
    ...
    public void insertOrder(Connection con, Order order) throws SQLException {
        QueryRunner r = new QueryRunner();
        String sql = "insert into `order`(total,amount,status,paytype,name,phone,address,datetime,user_id,delivery_type,pickup_time) values(?,?,?,?,?,?,?,?,?,?,?)";
        r.update(con,sql,
                ...
                order.getPickupTime() == null ? null : new java.sql.Timestamp(order.getPickupTime().getTime()) );
    }
