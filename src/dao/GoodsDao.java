...
//        return r.query(sql, new MapHandler());
    ...
    // ================ 优化修改：补充关联type表，查询typename（核心修改） ================
    // 功能：支持查询对应分类（含奶茶12）的商品，同时封装typename到Goods实体
    public List<Goods> selectGoodsByTypeID(int typeID,int pageNumber,int pageSize) throws SQLException {
        QueryRunner r=new QueryRunner(DataSourceUtils.getDataSource());
        if(typeID==0)
        {
            // 修改：关联type表，查询t.name as typename，与Goods实体的typename字段对应
            String sql="select g.*, t.name as typename from goods g LEFT JOIN type t ON g.type_id = t.id limit ? , ?";
            return  r.query(sql,new BeanListHandler<Goods>(Goods.class),(pageNumber-1)*pageSize,pageSize);
        }
        else
        {
            // 修改：关联type表，查询t.name as typename，支持奶茶系列（typeID=12）的分类名称封装
            String sql="select g.*, t.name as typename from goods g LEFT JOIN type t ON g.type_id = t.id where g.type_id=? limit ? , ?";
